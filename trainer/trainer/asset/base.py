...
class DailyIndicator:
    ...
    _ema: float  # Change in price expressed as a ratio compared to the previous day
...
class DailyAsset(ABC):
    ...
    def find_matched_tradable_date_range(
        ...
        for candle in self.__candles:
            ...
            # `- 1` indicates stopping skipping when we reach the end date
            if (
                excluding_historical
                and extra_historical_days_count <= historical_days_num + self.calc_buffer_days_num() - 1
            ):
                continue
