...
class ComposedSinusoid(DailyAsset):
    ...
    def __init__(
        ...
        fluctuation_scale_range: Tuple[float, float],
    ) -> None:
        DailyAsset.__init__(self, symbol)
        self._published_time = published_time
        components = []
        for _ in range(components_num):
            components.append(self._generate_sine(
                alpha_range, beta_range,
                gamma1_range, gamma2_range,
                fluctuation_scale_range,
            ))
    ...
    def _generate_sine(
        ...
        fluctuation_scale = np.random.uniform(*fluctuation_scale_range)
        return lambda time: sine(time, self._published_time, alpha, beta, gamma1, gamma2, fluctuation_scale)
...
def sine(
    ...
    y = math.sin(alpha * x + gamma1) * beta + gamma2
    y *= 1 + np.random.laplace(0, fluctuation_scale)
    return y
