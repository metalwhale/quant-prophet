...
class Zigzag(DailyAsset):
    ...
    _trend_type_weights: Tuple[float, float, float]  # (UP, DOWN)
    ...
    def _fetch_candles(self) -> List[DailyCandle]:
        trend_start_date: datetime.date = None
        trend_start_price: float = None
        ...
        for date, price in zip([d for d, _ in raw_prices], smoothen([p for _, p in raw_prices])):
