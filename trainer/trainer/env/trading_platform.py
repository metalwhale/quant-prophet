...
class TradingPlatform(gym.Env):
    ...
    #   and a "step" simply refers to a trading day.
    # "Date":
    ...
    # State components
    # Episode-level components, only changed if we reset to begin a new episode
    ...
    def step(self, action: np.int64) -> tuple[Dict[str, Any], SupportsFloat, bool, bool, dict[str, Any]]:
        ...
        if len(self._positions) == 0 or action != self._positions[-1].position_type.value:
            amount = self._POSITION_AMOUNT_UNIT if self._position_amount_type == AmountType.STATIC \
    ...
    def render(self) -> Any | List[Any] | None:
        ...
        figure = plt.figure(
            ...
            figsize=(len(dates) / YEARLY_TRADABLE_DAYS_NUM * YEAR_WIDTH, features_num * SUBPLOT_HEIGHT),
            dpi=400,
            clear=True,
        )
        ...
        # Plot prices
        axes = figure.add_subplot(features_num, 1, 1)
        all_axes.append(("price", axes, {p.date: p.actual_price for p in prices}))
        current_position_index: Optional[int] = None
        date_prices: List[Tuple[datetime.date, float]] = []
        for i, price in enumerate(prices):
            next_position_index = 0 if current_position_index is None else current_position_index + 1
            ...
            if (
                ...
                if len(date_prices) > 0:
                    ...
                    if current_position_index is None:
                        ...
                        current_position_type = PositionType.SIDELINE
                    else:
                        current_position_type = self._positions[current_position_index].position_type
                    color: str
                    if current_position_type == PositionType.SIDELINE:
                        color = "gray"
                    elif current_position_type == PositionType.BUY:
                        color = "green"
                    elif current_position_type == PositionType.SELL:
                        color = "red"
                    else:
        ...
        for i, feature in enumerate(FEATURES):
            ...
            axes = figure.add_subplot(features_num, 1, i + 2)
        ...
        axes = figure.add_subplot(features_num, 1, features_num)
    ...
    def set_mode(self, is_training: bool):
        ...
        self._position_amount_type = AmountType.STATIC if is_training else AmountType.DYNAMIC
    ...
    def _retrieve_prices(self):
        # Since `retrieve_historical_prices` chooses the last price from a random time on the same end date,
        # multiple calls produce different results for the last price.
        # We need to avoid calling it again if the current date matches when we last retrieved prices.
